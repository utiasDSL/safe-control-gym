...
class GPRC(BaseController):
    ...
    def __init__(
        ...
        **kwargs):
    	...
    	self.Q_h2 = get_cost_weight_matrix(q, self.state_dim)
    	self.R_h2 = get_cost_weight_matrix(r, self.input_dim)
    	self.H2_gain = None
    ...
    def preprocess_training_data(self, x_seq, u_seq, x_next_seq):
        ...
        x_pred_seq = self.prior_dynamics(x0=x_seq.T - self.x_0[:,np.new_axis],
                                               p=u_seq.T - self.u_goal[:,np.new_axis])['xf'].toarray()
        
        targets = (x_next_seq.T - (x_pred_seq + self.x_0[:,np.new_axis])).transpose()  # (N, nx).
    ...
    def sample_init_states(self):
		...
		limits = [(self.init_ctrl.env.INIT_STATE_RAND_INFO[key]['low'], self.init_ctrl.env.INIT_STATE_RAND_INFO[key]['high']) for key in
				 init_ctrl.env.INIT_STATE_RAND_INFO]
		samples = lhs_sampler.generate(limits, self.train_samples + self.val_samples, random_state=self.seed)
		init_state_samples = np.array(samples)

		return init_state_samples

	def rollout_ctrl(self, ctrl=None, init_state_samples, samples=self.val_samples):
		...
		if len(init_state_samples.shape)==1:
			init_state_samples = init_state_samples[np.new_axis, :]
	...
	def solve_H2_optim(self, x_0, u_0, beta=1.):
		...
		Bw = np.sqrt(gaussian_process.predict(self.query)[1].detach().numpy()) #Query prediction variances
		...
		if prob.status==optimal:
			Print(f"LMI optimization solved with beta = {beta}.")
	...
	def compute_H2_gain(self, x_0, u_0):
        ...
        for i in range(self.max_optim_tries):
	        try:
	        	R, Q = solve_H2_optim(x_0, u_0, beta)
	        	gain = R@np.linalg.inv(Q)
	        	break
	        except:
	        	beta += self.step_size
	        	print(f"H2 optimization infeasible, trying again with beta = {beta}")
    ...
    def init_lin_matrices(self, x_0, u_0):
	    ...
	    self.A =  self.discrete_dfdx + gp_jacobian[:,:-self.nu]
		self.B =  self.discrete_dfdu + gp_jacobian[:,-self.nu:]
		self.Au = gp_jacob_uncert[:,:-self.nu]
		self.Bu = gp_jacob_uncert[:,-self.nu:]
