...
class Controller():
    ...
    def get_x(self, step):
        ...
        t = step / 30
        u = step / 10
        return np.cos(t) * (R + r*np.cos(u)) * 2500.
    def get_y(self, step):
        R = 1
        r = 0.5
        t = step / 30
        u = step / 10
        return np.sin(t) * (R + r*np.sin(u)) * 2500.
    def get_z(self, step):
        R = 1
        r = 0.5
        t = step / 30
        u = step / 10
        return r*np.sin(u) * 2500.
    ...
    def cmdFirmware(self,
        ...
        if iteration == 0:
            ...
            target_rpy_rates = np.array([self.get_x(step), self.get_y(step), self.get_z(step)])
