...
Adapted from https://github.com/openai/spinningup/blob/master/spinup/algos/pytorch/ppo/ppo.py
Adapted hyperparameters from [here](https://github.com/DLR-RM/rl-baselines3-zoo/blob/master/hyperparams/ppo.yml) 
...
Example: 
    train on cartpole balance::  
        ...
        $ python mains/main.py --algo ppo --task cartpole --overrides ppo_cartpole.yaml 

    test on cartpole balance::

        $ python mains/main.py --func test --restore {...}

    train on quadrotor stabilization:: 
...
class PPO(BaseController):
    ...
    def __init__(self, env_func, training=True, checkpoint_path="model_latest.pt", output_dir="temp", device="cpu", seed=0, **kwargs):
    ...
    def reset(self):
        ...
        if self.training:
            ...
            self.eval_env.add_tracker("constraint_values", 0, mode="queue")
    ...
    def log_step(self, results):
        ...
        if "eval" in results:
            ...
            # each episodic constraint
            if "constraint_values" in results["eval"]:
                con_dict = {}
                con_vals = results["eval"]["constraint_values"]
                if len(con_vals.shape) == 2:
                    # multiple constraints
                    for i in range(con_vals.shape[-1]):
                        con_name = "constraint_value_{}".format(i)
                        con_dict[con_name] = con_vals[:, i].mean()
                else:
                    # single constraint
                    con_name = "constraint_value_0"
                    con_dict[con_name] = con_vals.mean()
                self.logger.add_scalars(con_dict, step, prefix="stat_eval")
