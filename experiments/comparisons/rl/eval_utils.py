...
def plot_from_hpo(legend_dir_specs,
                      out_path="temp.jpg",
                      scalar_names=[],
                      title="Hyperparameter Optimization Evaluation",
    ...
    bplot = ax.boxplot(last_step_stats,
                        vert=True,
                        patch_artist=True,
                        labels=x_cat)
    
    ax.set_title('HPO evaluation')
    ax.set_yscale('log')
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabels)

    plt.savefig(out_path)
    return scalar_stats, last_step_stats, x_cat

def plot_from_hp_sens(legend_dir_specs,
    ...
    plt.savefig(os.path.join(out_path, xlabel + ".jpg"))
    # Write to csv.
    csv_path = os.path.join(out_path, xlabel + ".csv")
    os.makedirs(os.path.dirname(csv_path), exist_ok=True)
    with open(csv_path, "w") as f:
        writer = csv.writer(f)
        writer.writerow(x_cat)
        writer.writerows(last_step_stats)
    
    return scalar_stats, x_cat, last_step_stats
