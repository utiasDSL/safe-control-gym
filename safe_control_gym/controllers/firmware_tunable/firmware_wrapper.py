...
class FirmwareWrapper(BaseController):
    ...
    def __init__(self, 
        ...
        Args: 
            ...
            tunable_parameters (list): list of tunable parameters. Used when optimizing performance against real world data 
                [
                    ACTION_DELAY
                    SENSOR_DELAY
                    M_SCALE
        ...
        """
        # assert(len(tunable_parameters) == 8), "Requires 8 tunable parameters."
        super().__init__(env_func, **kwargs)
        self.firmware_freq = firmware_freq
        self.ctrl_freq = ctrl_freq

        # [0.69558162 0.33697152 0.7072021  0.19248974], 5.573174789807173, from basin_hopping
        # [0.26797431 0.53093888 0.74088065 0.49096269], 5.373037675488901, from evolution 

        self.ACTION_DELAY = max(0, int(tunable_parameters[0]*5))
        self.SENSOR_DELAY = max(0, int(tunable_parameters[1]*5))
        self.M_SCALE = 1 #int(tunable_parameters[2]*10)
        self.M_CONST = 0 #int(tunable_parameters[3]*10000)
        self.GYRO_SCALE = int(tunable_parameters[2]*10)
        self.GYRO_CONST = int(tunable_parameters[3]*10000)
        self.ACC_SCALE = 1 #int(tunable_parameters[6]*10)
        self.ACC_CONST = 0 #int(tunable_parameters[7]*0.1)
    ...
    def step(self, sim_time, action):
        ...
        while self.tick / self.firmware_freq < sim_time + self.ctrl_dt:
            ...
            if self.SENSOR_DELAY:
                ...
                self.sensor_history = self.sensor_history[1:] + [[self.ACC_SCALE*body_rot.apply(cur_acc) + self.ACC_CONST, self.GYRO_SCALE * cur_rotation_rates * self.RAD_TO_DEG + self.GYRO_CONST]]
            else:
                self._update_sensorData(sensor_timestamp, self.ACC_SCALE*body_rot.apply(cur_acc) + self.ACC_CONST, self.GYRO_SCALE * cur_rotation_rates * self.RAD_TO_DEG + self.GYRO_CONST)
            ...
            # Get action 
            new_action = self.KF * (self.PWM2RPM_SCALE * np.clip(self.M_SCALE * np.array(self.pwms) + self.M_CONST, self.MIN_PWM, self.MAX_PWM) + self.PWM2RPM_CONST)**2
