...
class HPO(object):
    ...
    def __init__(self, algo, task, load_study, output_dir, task_config, hpo_config, algo_config, safety_filter=None, sf_config=None):
        ...
        assert isinstance(hpo_config.hps_config.state_weight, list), 'state_weight must be a list'
        assert isinstance(hpo_config.hps_config.action_weight, list), 'action_weight must be a list'
    ...
    def checkpoints(self):
        ...
        if self.hpo_config.dashboard and self.hpo_config.use_database:
            ...
            run_server('sqlite:///{}_optuna.db'.format(self.study_name))
