...
class GeoController():
    ...
    def _compute_desired_force_and_euler(self,
        ...
        a_fb = -np.dot(k_pos,pos_e) - np.dot(K_vel,vel_e)
        a_des = a_fb + target_acc + (self.grav * cur_pos[2])
        desired_acc = a_des
            
        #---------Task 2: Compute the desired thrust command--------#
        c_cmd = self.mass * np.linalg.norm(a_des)
        ...
        z_bdes = a_des / np.linalg.norm(a_des)
